...
while true; do
  ...
  if json_expr_true "$status_json" 'isinstance(data.get("last_completed_job"), dict) and data.get("last_completed_job", {}).get("status") == "succeeded"'; then
    break
  fi

  if json_expr_true "$status_json" 'isinstance(data.get("last_completed_job"), dict) and data.get("last_completed_job", {}).get("status") == "failed"'; then
...
keyword_json="$(http_get_json "$(backend_url '/api/search?query=Sherlock&limit=5')")"
